...
public class Test25 {
    ...
    /* 
            @MatrixVariable annotation을 사용하려면 스프링 설정을 활성화 시켜야 한다.
            1) XML 설정
...
http://localhost:8888/app/ex03/test25/search2/;car=test;color=Black;auto=true;year=2019
  ..?
    ...
    public String m3(
            //  PathVariable이 다르더라도
            //  MatrixVariable 이름으로 값을 추출할 수 있다.
            @MatrixVariable String car,
            @MatrixVariable String color,
            @MatrixVariable Boolean auto,
            @MatrixVariable int year,
    ...
    @RequestMapping(value="search4/car/{value1}/spec/{value2}",
                    produces="text/plain;charset=UTF-8")
    public String m4(
            //  PathVariable이 다르고, MetrixVariable 이름이 같을 때?
            //  PathVariable과 MatrixVariable 이름을 명확하게 해야한다.
