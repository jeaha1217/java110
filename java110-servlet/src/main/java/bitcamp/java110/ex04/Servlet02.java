...
// POST 요청
// => POST 요청으로 값으 보내기 위해서는 [message-body]에 붙여 보내야한다.
...
form data name=asd&age=123&working=true
*/
...
/*
    ...
    - 바이너리 데이터 전송
        => message-body 부분에 첨부하기 때문에 바이너리 데이터를 전송할 수 있다.
            단, 멀티파트 형식으로 보낸다.
    - 적용 부분
        => 로그인, 회원가입 처럼 데이터가 캐시에 보관되거나 노출되지 말아야 하는경우.
        => 게시글 등록/변경처럼 대용량의 데이터를 전송 해야 하는 경우.
