/*  의존 객체 자동 주입 : @Autowired 애노테이션과 BeanPostProcessor
    ...
    =>  단, 스프링 IoC 컨테이너는 기본으로 @Autowired를 처리하지 않는다.
        ...
        도우미 객체를 스프링 IoC컨테이너에 장착해야 한다.
        꼭 String을 java.sql.Date으로 바꾸기 위해
        커스텀 프로퍼티 에디터를 장착하는 것 처럼.
        
    =>  @Autowired 애노테이션 처리기를 장착하는 방법.
        AutowiredAnnotationBeanPostProcessor 이 클래스의 객체를 등록하라!
        그냥 등록만 하면 된다.
        그러면 Spring IoC Container가 알아서 이 객체를 사용하여 @Autowired를 처리한다.
        
        호? 원리?
        
    BeanPostProcessor?
    =>  Spring IoC Container틑 객체를 생성한 후 BeanPostProcessor를 실행한다.
