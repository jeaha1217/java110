/*  "Custom Property Editor"를 정의하고 설정하는 방법. 

    =>  스프링 IoC 컨테이너가 객체를 만들기 위해 생성자를 호출할 때
        또는 프로퍼티 값을 설정하기 위해 setter를 호출할 때,
        XML 설정 파일에 지정된 문자열을 해당 파라미터 타입에 맞게 자동으로 형변환한다.
        단, primitive type만 자동 형변환 한다.
    
    예) String ==> byte/short/int/long/float/double/boolean/char
 
    =>  그 외 타입에 대해 형변환 하려면 별도의 변환기를 장착해야 한다.
        그 별도의 값 변환기를 "커스텀 프로퍼티 에디터(Custom Property Editor)"라고 한다. 
    
    커스텀 프로퍼티 에디터 만들기.
    =>  java.beans.PropertyEditorSupport 인터페이스를 구현.
        -> 구현할 메서드가 너무 많기 때문에 보통 이 인터페이스를 직접 구현 하기 보다는
    ...
    =>  커스텀 에디터를 Spring IoC 컨테이너에 등록한다.
 */
package ex07;
...
//        System.out.println(c2);
        ...
        map.put(java.sql.Date.class, value)
        */
    }
}
