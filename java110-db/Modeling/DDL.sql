...
CREATE TABLE p2_lect (
    ...
    crno  INTEGER      NULL     COMMENT '강의실 번호', -- 강의실 번호
    mno   INTEGER      NULL     COMMENT '매니져 번호' -- 매니져 번호
)
COMMENT '강의';
...
CREATE TABLE p2_cls_room (
    crno INTEGER     NOT NULL COMMENT '강의실 번호', -- 강의실 번호
...
ALTER TABLE p2_cls_room
    MODIFY COLUMN crno INTEGER NOT NULL AUTO_INCREMENT COMMENT '강의실 번호';

-- 학생
CREATE TABLE p2_stud (
    sno   INTEGER      NOT NULL COMMENT '회원 번호', -- 회원 번호
...
CREATE TABLE p2_tchr (
    tno  INTEGER      NOT NULL COMMENT '회원 번호', -- 회원 번호
...
-- 매니져
ALTER TABLE p2_mgr
    ADD CONSTRAINT PK_p2_mgr -- 매니져 기본키
        PRIMARY KEY (
            mno -- 회원 번호
        );
...
CREATE TABLE p2_cr_phot (
    ...
    phot VARCHAR(255) NOT NULL COMMENT '사진 파일명', -- 사진 파일명
    crno INTEGER      NOT NULL COMMENT '강의실 번호' -- 강의실 번호
)
COMMENT '강의실 사진';
...
ALTER TABLE p2_cr_phot
    ADD CONSTRAINT PK_p2_cr_phot -- 강의실 사진 기본키
        PRIMARY KEY (
            cpno -- 강의실 사진 번호
        );
...
ALTER TABLE p2_tchr_lect
    ADD CONSTRAINT PK_p2_tchr_lect -- 강사 배정 기본키
        PRIMARY KEY (
            tno, -- 회원 번호
            lno  -- 강의번호
        );
...
ALTER TABLE p2_tchr_subj
    ADD CONSTRAINT PK_p2_tchr_subj -- 강의 과목 기본키
        PRIMARY KEY (
            sjno, -- 과목번호
            tno   -- 회원 번호
        );
...
CREATE TABLE p2_memb (
    uno      INTEGER      NOT NULL COMMENT '회원 번호', -- 회원 번호
...
ALTER TABLE p2_memb
    MODIFY COLUMN uno INTEGER NOT NULL AUTO_INCREMENT COMMENT '회원 번호';

-- 강의
ALTER TABLE p2_lect
    ADD CONSTRAINT FK_p2_mgr_TO_p2_lect -- 매니져 -> 강의
        FOREIGN KEY (
            mno -- 매니져 번호
        )
        REFERENCES p2_mgr ( -- 매니져
            mno -- 회원 번호
        );
...
-- 매니져
ALTER TABLE p2_mgr
    ADD CONSTRAINT FK_p2_memb_TO_p2_mgr -- 회원 -> 매니져
        FOREIGN KEY (
            mno -- 회원 번호
        )
        REFERENCES p2_memb ( -- 회원
...
-- 강의실 사진
ALTER TABLE p2_cr_phot
    ADD CONSTRAINT FK_p2_cls_room_TO_p2_cr_phot -- 강의실 -> 강의실 사진
        FOREIGN KEY (
            crno -- 강의실 번호
        )
        REFERENCES p2_cls_room ( -- 강의실
...
-- 강사 배정
ALTER TABLE p2_tchr_lect
    ADD CONSTRAINT FK_p2_tchr_TO_p2_tchr_lect -- 강사 -> 강사 배정
        FOREIGN KEY (
            tno -- 회원 번호
        )
...
-- 강사 배정
ALTER TABLE p2_tchr_lect
    ADD CONSTRAINT FK_p2_lect_TO_p2_tchr_lect -- 강의 -> 강사 배정
        FOREIGN KEY (
            lno -- 강의번호
        )
...
-- 강의 과목
ALTER TABLE p2_tchr_subj
    ADD CONSTRAINT FK_p2_tchr_TO_p2_tchr_subj -- 강사 -> 강의 과목
        FOREIGN KEY (
            tno -- 회원 번호
        )
        REFERENCES p2_tchr ( -- 강사
...
-- 강의 과목
ALTER TABLE p2_tchr_subj
    ADD CONSTRAINT FK_p2_subj_TO_p2_tchr_subj -- 과목 -> 강의 과목
        FOREIGN KEY (
            sjno -- 과목번호
        )
