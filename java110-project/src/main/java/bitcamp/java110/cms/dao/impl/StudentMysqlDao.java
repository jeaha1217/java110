...
public class StudentMysqlDao implements StudentDao {
    ...
    public List<Student> findAll() throws DaoException {
        ...
        try {
            ...
            rs = stmt.executeQuery(
                    "select" +
                    " m.mno, m.name, m.email, s.schl, s.work " +
                    " from p1_stud s" + 
    ...
    public Student findByEmail(String email) throws DaoException {
        ...
        try {
            ...
            rs = stmt.executeQuery(
                    "select" +
                    " m.mno, m.name, m.email, s.schl, s.work, mp.photo" +
                    " from p1_stud s" + 
                    " inner join p1_memb m on s.sno = m.mno" +
    ...
    public Student findByNo(int no) throws DaoException {
        ...
        try {
            ...
            rs = stmt.executeQuery(
                    "select" +
                    "  m.mno,  m.name,  m.email,  m.tel,  s.schl,  s.work,  mp.photo" +
                    " from p1_stud s" + 
                    " inner join p1_memb m on s.sno = m.mno" +
                    " left outer join p1_memb_phot mp on s.sno=mp.mno" +
                    " where m.mno=" + no);
    ...
    public Student findByEmailPassword(String email, String password) throws DaoException {
        ...
        try {
            ...
            rs = stmt.executeQuery(
                    "select" +
                    " m.mno, m.name, m.email, m.tel, s.schl, s.work" + 
                    " from p1_stud s" + 
                    " inner join p1_memb m on s.sno = m.mno" +
                    " where m.email='" + email + 
